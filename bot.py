...
class Bot(object):
	...
	def __init__(self):
		...
		self.oauth = {"client_id": data["CLIENT_ID"],
					  "client_secret": data["CLIENT_SECRET"],
					  # Scopes provide and limit permissions to what our app
					  # can access. It's important to use the most restricted
					  # scope that your app will need.
					  "scope": "bot"}
		self.verification = data["VERIFICATION_TOKEN"]

		# NOTE: Python-slack requires a client connection to generate
		# an oauth token. We can connect to the client without authenticating
		# by passing an empty string as a token and then reinstantiating the
		# client with a valid OAuth token once we have one.
		self.client = SlackClient(data["ACCESS_TOKEN"])
	...
	def send_message(self, team_id, user_id, channel, txt, node_id="unknown", dm=False):
	...
	def onboarding_message(self, team_id, user_id):
		...
		#print "auth test"
		#print self.client.api_call("auth.test")
		#print "auth test"
